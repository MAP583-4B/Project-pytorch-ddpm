...
class CelebADataset(torch.utils.data.Dataset):
    def __init__(self, dataset_dir):
        ...
        self.images_dir = os.path.join(dataset_dir, "img_align_celeba")
        self.annotations_dir = os.path.join(dataset_dir, "annotations")
        self.image_filenames = os.listdir(self.images_dir)
        self.annotations_filename = os.path.join(self.annotations_dir, "list_attr_celeba.txt")
        self.annotations = np.genfromtxt(self.annotations_filename, skip_header=1, usecols=range(1, 41), dtype=int)
    ...
    def __getitem__(self, idx):
        ...
        image = Image.open(os.path.join(self.images_dir, self.image_filenames[idx])).convert('L')

        # Load the annotations for the image
        annotations = self.annotations[idx]

        # Convert the image and annotations to PyTorch tensors
...
def train():
    ...
    dataset = CelebA(
        ...
        """
    """
    dataset = datasets.ImageFolder('data/img_align_celeba', transform=transforms.Compose([
            transforms.RandomHorizontalFlip(),
            transforms.ToTensor(),
            transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5)),
        ]))
    ...
    dataset = CelebADataset(dataset_dir)
    dataset = MyDataset([(Image.fromarray(transform(x[0].numpy())),x[1]) for x in dataset])
    ...
    #writer.add_image('real_sample', grid.squeeze())
    writer.flush()
